...
public final class BlockStateHelper {
    ...
    static final class ResourceTemplates {
    ...
    public static ModelFile fruitLog(BlockStateProvider provider, Block block) {
        ...
        final ResourceLocation texture = isWood
                ? ResourceLocation.fromNamespaceAndPath(ownTexture.getNamespace(),
                        "block/" + blockName.substring(0, blockName.length() - "_wood".length())
                                + "_log")
                : ownTexture;
        ...
        if (isWood) {
            ...
            base = provider.models().getExistingFile(ResourceLocation
                    .fromNamespaceAndPath(ownTexture.getNamespace(), "block/" + logName));
            noside = provider.models().getExistingFile(ResourceLocation.fromNamespaceAndPath(
                    ownTexture.getNamespace(), "block/" + logName + "_noside"));
            side = provider.models()
                    .withExistingParent(blockName + "_side", ResourceTemplates.FRUIT_WOOD_SIDE)
                    .texture("texture", texture);
            otherside = provider.models()
                    .withExistingParent(blockName + "_otherside",
                            ResourceTemplates.FRUIT_WOOD_OTHERSIDE)
                    .texture("texture", texture);
        } else {
            base = provider.models().withExistingParent(blockName, ResourceTemplates.FRUIT_LOG)
                    .texture("texture", texture);
            side = provider.models()
                    .withExistingParent(blockName + "_side", ResourceTemplates.FRUIT_LOG_SIDE)
                    .texture("texture", texture);
            otherside = provider.models()
                    .withExistingParent(blockName + "_otherside",
                            ResourceTemplates.FRUIT_LOG_OTHERSIDE)
                    .texture("texture", texture);
            noside = provider.models()
                    .withExistingParent(blockName + "_noside", ResourceTemplates.FRUIT_LOG_NOSIDE)
                    .texture("texture", texture);
        }
        ...
        builder.part().modelFile(otherside).rotationX(90).addModel()
                .condition(axis, Direction.Axis.Z).condition(north, false).condition(south, false)
                .end();
        builder.part().modelFile(otherside).rotationX(90).addModel()
        ...
        if (leafy != null) {
            ...
            final ModelFile leaves = provider.models().getExistingFile(
                    ResourceLocation.fromNamespaceAndPath(texture.getNamespace(),
                            "block/" + treeName + "_leaves"));
            builder.part().modelFile(leaves).uvLock(true).addModel().condition(leafy, true).end();
        }

        return provider.models()
                .withExistingParent(blockName + "_item",
                        isWood ? ResourceTemplates.FRUIT_WOOD_ITEM
                                : ResourceTemplates.FRUIT_LOG_ITEM)
                .texture("texture", texture);
    }
}
